...
public class HttpDownloadTask implements Runnable {
    ...
    private void saveFile(InputStream in) throws IOException {
        ...
        System.out.println(String.format("file path %s",fileName.getAbsoluteFile().toString()));
    ...
    public static void main(String[] args) {
        ...
        DownerConfigure configure =new DownerConfigure.Builder().build();
...
//        new Thread(new HttpDownloadTask(node,configure)).start();
        for (GraphNode item:
                nodes) {
//            pool.submit(new Thread(new HttpDownloadTask(item,configure)));
            pool.submit(new HttpDownloadTask(item,configure));
        }
    }
